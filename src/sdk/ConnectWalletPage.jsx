...
function ConnectWalletPage() {
  ...
  const dexrootAddr = config.dexroot;
  ...
  async function genSeed(pin) {
    const randMnemonic = await client.crypto.mnemonic_from_random({
      word_count: 12,
    });

    const arr = randMnemonic.phrase.split(" ");
    ...
    console.log("crypt");
    const encrypted = aes.encryptText(mnemonic, pin);
  ...
  async function getClientAddrAtRootForShard(pubkey, n) {
    ...
    try {
      ...
      console.log(1);
      const {value0} = response.decoded.output;
  ...
  async function onSharding(pubkey) {
    ...
    try {
      const targetShard = getShard(dexrootAddr);
      console.log("pubkeypubkey", pubkey);
      let status = false;
      let n = 0;
      let clientAddress;
      while (!status) {
        const response = await getClientAddrAtRootForShard(pubkey, n);
        console.log("response", response);
        const clientAddr = response;
        const shard = getShard(clientAddr);
        if (shard === targetShard) {
          ...
          setAddr(clientAddr);
          walletStore.setAddress(clientAddr);
          return {
            status: true,
  ...
  async function prepareClientDataForDeploy(phrase) {
    ...
    walletStore.setPubkey(clientKeys.public);
    walletStore.setSecret(clientKeys.secret);
    const clientPubkey = clientKeys.public;
  ...
  async function deployClient(clientSet, clientKeys) {
    ...
    const checkAddress = clientSet.data.address === address;
    console.log(
      checkAddress,
  ...
  function NextPage() {
    ...
    if (curentPage === 2) {
      if (
        ...
        setCurentPage(curentPage + 1);
        const temp = [input1, input2, input3, input4];
        pass = temp.join("");

        walletStore.setPin(pass);
        genSeed(pass);
      } else {
        setErrorModal([
          {
  ...
  function NextPageLogin() {
    ...
    if (curentPageLogin === 1) {
      if (seedLogin !== "") {
        setCurentPageLogin(curentPageLogin + 1);
        walletStore.setSeedPhrase(seedLogin);
      } else {
        setErrorModal([
          {
            hidden: true,
    ...
    if (curentPageLogin === 2) {
      if (
        ...
        const temp = [inputL1, inputL2, inputL3, inputL4];
        const pass = temp.join("");
        walletStore.setPin(pass);

        const trim = seedLogin.trim();
        setLoader(true);

        const promiseKeys = getClientKeys(trim);
        promiseKeys.then(
          (data) => {
            walletStore.setPubkey(data.public);
            walletStore.setSecret(data.secret);
            const addr = data.public;
            const promiseAddr = onSharding(addr);

            promiseAddr.then(
              (data) => {
                walletStore.setAddress(data.data.address);
                const addr = data.data.address;
                const promiseAcc = getAccType2(data.data.address);

                promiseAcc.then(
                  (data) => {
                    const acc = data.acc_type;
                    console.log(acc);
                    if (acc === 1) {
                      setLoader(false);

                      const encrypted = aes.encryptText(trim, pass);
  ...
  function testdep() {
    const bal = getClientBalance(addr);
    setLoader(true);
    bal.then(
      (data) => {
        // console.log(bal);
...
export default observer(ConnectWalletPage);
